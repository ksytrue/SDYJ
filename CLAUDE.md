...
1. **초기 Ver.2**: 서현고 사진(`images/background.jpg`)을 네비게이션 로고(36x36)와 히어로 배경으로 사용. 홈에 해설 영상/학습 매거진/팀 프로젝트 3개 카드 배치
2. **UI 수정 (2차)**: 아래 3가지 변경 적용
   - 팀 프로젝트 카드 삭제 → 해설 영상 + 학습 매거진 2개만 유지
   - 히어로 섹션의 큰 배경 이미지(`background.jpg`) 제거 → 파란색 그라데이션 배경으로 대체
   - 네비게이션/히어로/푸터 로고를 이미지 대신 CSS 마름모(◆) 형태로 변경 (`transform: rotate(45deg)`)
3. **LaTeX 추가**: KaTeX CDN을 통한 수식 렌더링 지원. 매거진 본문과 영상 설명에서 `$...$`(인라인), `$$...$$`(블록) 문법 사용 가능
...
js/app.js               # 핵심 로직. 탭 전환, 모달, fetch+렌더링, renderLatex() 함수
