...
### Ver.2.3 — 공지사항 탭 추가 (2025-06-23)
- 네비게이션에 **공지사항** 탭 신설
- 공지사항 탭 진입 시 `README.md` 파일을 자동으로 불러와 렌더링
- README.md에 업데이트 내역 섹션 추가

### Ver.2.2 — LaTeX 수식 지원 (2025-06-23)
- KaTeX CDN 추가 (CSS + JS)
- 매거진 본문(`.md` 파일)과 영상 설명에서 LaTeX 수식 렌더링 지원
- 인라인 수식(`$...$`), 블록 수식(`$$...$$`) 문법 사용 가능

### Ver.2.1 — UI 수정 (2025-06-23)
- 홈 화면의 **팀 프로젝트 카드 삭제** (해설 영상 + 학습 매거진 2개만 유지)
- 히어로 섹션의 큰 배경 이미지 제거 → 파란색 그라데이션 배경으로 대체
- 네비게이션/히어로/푸터 로고를 이미지 대신 **CSS 마름모(◆) 형태**로 변경

### Ver.2.0 — 전면 리디자인 (2025-06-23)
- 기존 다중 HTML 페이지 구조를 **단일 페이지 탭 기반 SPA**로 전면 개편
- 모던하고 깔끔한 UI 디자인 적용 (반응형: 모바일/태블릿/데스크톱)
- **매거진 시스템**: `magazines/` 폴더에 마크다운 파일을 넣고 `js/magazines.js`에 등록
- **영상 시스템**: 그리드 레이아웃 + 상세 모달 (제목, 제작자, 설명, 영상 인덱스)
- `magazine.html`, `midterm.html`, `style.css` 삭제 (탭으로 통합)

### Ver.1.0 — 초기 버전
- `index.html`, `magazine.html`, `midterm.html` 다중 페이지 구조
- 기본적인 영상 재생 및 매거진 페이지 제공
